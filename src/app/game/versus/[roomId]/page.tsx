...
export default function VersusGamePage() {
  ...
  const { isHost, players, roomStatus, joinExisting, setReady, startGame, setRoomStatus } = useRoomStore();
  const { status: gameStatus, hp, score, combo, orders, currentBurger,
    startGame: startLocalGame } = useGameStore();

  const [opponent, setOpponent] = useState<OpponentState>({ hp: 80, queueCount: 3, status: 'playing' });
  ...
  const { sendStateUpdate, sendAttack } = useVersusRoom(roomId, playerId ?? '', handleOpponentUpdate);
  useLobbyRoom(roomId);
  useGameLoop();
      ...
      <div className="multi-hub">
        <div className="room-lobby">
          ...
          <div className="room-lobby__players">
            ...
            {players.length < 2 && (
              <p style={{ fontFamily: 'Mulmaru', fontSize: '0.75em', color: '#7a7a9a' }}>상대방 대기 중...</p>
            )}
          </div>
          {!myReady && (
          ...
          {isHost && (
            <button className="btn btn--primary" onClick={handleStart} disabled={!allReady}>
              {allReady ? '게임 시작' : '대기 중...'}
            </button>
          )}
        </div>
        <button className="btn btn--ghost" onClick={() => router.push('/')}>취소</button>
      </div>
    );
  }

  return (
    <div className="ingame" style={{ display: 'flex', flexDirection: 'column' }}>
      {/* 상단: 상대방 미니 패널 */}
      <div className="versus-opponent">
        <span>{opponentEntry?.nickname ?? '상대방'}</span>
        <span>HP {Math.ceil(opponent.hp)}</span>
        <span>큐 {opponent.queueCount}</span>
        {opponent.status === 'gameover' && <span style={{ color: '#4caf50' }}>GAME OVER</span>}
      </div>
